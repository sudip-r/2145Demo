// Script assets have changed for v2.3.0 see
// https://help.yoyogames.com/hc/en-us/articles/360005277377 for more information
function PlayerStateRun(){
	// Running becomes unavailable once strength is fully drained. This flag (not the raw
	// strength value) is what blocks running again, so a couple of regen frames can't
	// immediately flip the player back into PlayerStateRun and drain it right back to 0.
	if(global.playerStrength <= 0)
	{
		global.playerExhausted = true;
	}

	// Stop running when shift is released, movement input stops, or the player is exhausted.
	if(!keyRun || inputMagnitude == 0 || global.playerExhausted)
	{
		state = PlayerStateFree;
		PlayerStateFree();
		exit;
	}

	// Convert input direction into run-speed movement.
	hSpeed = lengthdir_x(inputMagnitude * speedRun, inputDirection);
	vSpeed = lengthdir_y(inputMagnitude * speedRun, inputDirection);

	PlayerCollision();

	// Running steadily drains strength while actually moving.
	global.playerStrength = max(0, global.playerStrength - strengthDrainRun);

	// Choose run sprite while moving, otherwise fall back to idle.
	var _oldSprite = sprite_index;

	if(inputMagnitude != 0)
	{
		direction = inputDirection;
		sprite_index = spriteRun;
	} else sprite_index = spriteIdle;

	// Restart animation when changing between movement and idle sprites.
	if(_oldSprite != sprite_index) localFrame = 0;

	PlayerAnimateSprite();

	// Space first talks to nearby NPCs, then falls back to the active inventory item.
	if(keyActivate)
	{
		// Search in front of the moving player for an NPC interaction target.
		var _npcTarget = NpcFindInteractTarget(id);

		// If an NPC was found, leave run state and trigger their placeholder interaction.
		if(_npcTarget != noone)
		{
			state = PlayerStateFree;
			NpcInteract(_npcTarget, id);
			exit;
		}

		// No NPC nearby - let the active inventory slot decide what the action button does.
		PlayerUseActiveItem();
	}
}
